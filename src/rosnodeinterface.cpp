...
// C/C++ source code generated on : Wed Nov 20 03:37:46 2024
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"l5\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(l5_M, (NULL));
        l5_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(l5_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(l5_M)
                == (NULL)) || rtmGetStopRequested(l5_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(l5_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
